    ...
    public class Map
    {
        ...
        private void PositionPlayer(Player player)
        {
            ...
            do
            {
                ...
                //kontrola kolizie medzi hracmi
                if (!hit && _game.PlayersInGame.IndexOf(player) != 0)
                {
                    foreach (var player_ in _game.PlayersInGame)
                    {
                        if (player != player_)
                        {
                            var difX = Math.Abs(player_.Body.Position.X + player_.Body.Origin.X - (player.Body.Position.X + player.Body.Origin.X)) - (player_.Body.Size.X / 2 + player.Body.Size.X / 2);
                            //prekrytie hracov
                            /*if (player.GetCollider().CheckCollision(player_.GetCollider(), 0, false) != default)
                            {
                                Console.WriteLine($"Collision of {player_.Name} with player: {player.Name}");
                                hit = true;
                                break;
                            }*/
                            //hraci nad sebou
                            if (difX < 0f && player.Y != player_.Y)
                            {
                                Console.WriteLine($"Elimination between {player_.Name} and {player.Name}");
                                //kontrola, ci je medzi nimi platforma
                                foreach (var platform in Platforms)
                                {
                                    difX = Math.Abs(platform.Body.Position.X + platform.Body.Origin.X - (player.Body.Position.X + player.Body.Origin.X)) - (platform.Body.Size.X / 2 + player.Body.Size.X / 2);
                                    if (difX < 0f) //difX2 < 0f &&
                                    {
                                        if (!((player.Y < platform.Y && platform.Y < player_.Y) || (player_.Y < platform.Y && platform.Y < player.Y)))
                                        {
                                            Console.WriteLine($"Platform is not between players {player.Name} {player_.Name}");
                                            Console.WriteLine($"x: {platform.X}, y: {platform.Y}");
                                            hit = true;
                                            break;
                                        }
                                    }
