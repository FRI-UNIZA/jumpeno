...
public class User {
    ...
    public static List<Error> ValidateName(string name) {
        ...
        if (!Checker.IsAlphaNum(name)) errors.Add(new Error(NAME_ID, "Name must be alphanumeric"));
        if (name.Length > 0 && char.IsDigit(name[0])) errors.Add(new Error(NAME_ID, "Name must not start with number"));
        return errors;
    }
    ...
    public static List<Error> ValidateUnknown(User user) {
        var errors = new List<Error>();
        if (UNKNOWN.Equals(user)) errors.Add(new Error("User undefined!"));
        return errors;
    }
    public static void CheckUnknown(User user) {
        var errors = ValidateUnknown(user);
        if (errors.Count > 0) throw new ArgumentException(errors[0].Message);
    }
