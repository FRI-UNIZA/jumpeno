...
public class Player : Spectator, IRectFQuadStorable, IUpdateable, IRenderableParametric<Game> {
    // Attributes -------------------------------------------------------------------------------------------------------------------------
    ...
    // Constructors -----------------------------------------------------------------------------------------------------------------------
    [JsonConstructor]
    private Player(byte id, User user, DEVICE_TYPE device, bool connected, Body body, int kills): base(user, device) {
        ID = id;
        Connected = connected;
        Body = body;
        Kills = kills;
    ...
    public Player(byte id) : this(id, User.UNKNOWN, DEVICE_TYPE.POINTER, false, new(), 0) {}
    ...
    // Actions ----------------------------------------------------------------------------------------------------------------------------
    public void ConnectUser(User user, DEVICE_TYPE device) {
        User = user;
        Connected = true;
        Device = device;
    }
