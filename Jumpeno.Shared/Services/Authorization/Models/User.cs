...
public class User {
    ...
    public static List<Error> ValidateName(string name) {
        ...
        Checker.Validate(errors, !Checker.IsAlphaNum(name), new Error(nameof(Name), "Name must be alphanumeric"));
        Checker.Validate(errors, name.Length > 0 && char.IsDigit(name[0]), new Error(nameof(Name), "Name must not start with number"));
        return errors;
    }
