...
public class User {
    ...
    // Lifecycle --------------------------------------------------------------------------------------------------------------------------
    [JsonConstructor]
    private User(Guid? id, string? email, string name, SKIN skin) {
        ID = id;
        Email = email;
        Name = UserValidator.CheckName(name, null, false);
        Skin = UserValidator.CheckSkin(skin);
    }
    public User(Guid id, string email, string name, SKIN skin) : this((Guid?) id, email, name, skin) {}
    public User(string name, SKIN skin) : this(null, null, name, skin) {}
}
