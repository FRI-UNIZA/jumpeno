...
public class HTTP : StaticService<HTTP> {
    ...
    private static async Task<HTTPHeadResult> Request<T>(
        ...
        for (int iteration = 1; iteration <= 2; iteration++) {
            ...
            try {            
                ...
                // Add content headers:
                SetContentHeader(request, Header.CONTENT_TYPE, ContentType.JSON);
                ...
                throw Exceptions.REQUEST_CANCELLED;
            } catch {
                throw Exceptions.REQUEST_FAILED;
            } finally {
                cts.Dispose();
            }
            ...
            if (response.IsSuccessStatusCode && response is not null) {
                ...
                try {
                    ...
                    var exception = Exceptions.PARSING_ERROR
                ...
                if (isLocalURL && exception.Code == Exceptions.NOT_AUTHENTICATED.Code) await OnRefresh(iteration, exception);
    ...
    public static async Task<bool> Try(Func<Task> callback, string? form = null) {
        return await TabLock(new(async () => {
            ...
            catch (AppException e) { if (e.Code != Codes.REQUEST_CANCELLED) await OnError(e, form); }
            catch (AggregateException e) {
                foreach (var inner in e.InnerExceptions) {
                    if (inner is AppException app && app.Code == Codes.REQUEST_CANCELLED) continue;
